"""
ENERGY — Reporte Diario de Órdenes de Compra
=============================================
Lee el informe HTML de OC activas desde la carpeta Energy_bot
...
# ─── 2. ENCONTRAR EL INFORME OC MÁS RECIENTE ─────────────────────────────────

def find_oc_html(base_dir: str) -> str:
    """Busca el archivo Informe_OC_Activas*.html más reciente en la carpeta."""
    pattern = os.path.join(base_dir, 'Informe_OC_Activas*.html')
    files = glob.glob(pattern)
    if not files:
        print("ERROR: No se encontró ningún archivo Informe_OC_Activas*.html")
        sys.exit(1)
...
def load_oc_html(html_path: str) -> str:
...
<div style="max-width:860px;margin:0 auto;padding:16px;">
  <div style="background:#0F2B5B;padding:18px 28px;border-radius:12px 12px 0 0;display:flex;justify-content:space-between;align-items:center;">
    <div>
      ...
      <div style="font-size:11px;color:#93c5fd;margin-top:2px;text-transform:uppercase;letter-spacing:1px;">Reporte Diario — Órdenes de Compra</div>
    </div>
...
if __name__ == '__main__':
    ...
    html_path = find_oc_html(base_dir)
    html_content = load_oc_html(html_path)
    print(f"✅ Informe OC cargado ({len(html_content):,} caracteres)")
    ...
    subject  = f"📋 Informe OC Activas E&G — {date_str}"
    ...
    print("🎉 Informe OC enviado exitosamente.")
