...
def generate_html(inv: list, stats: dict, date_str: str, rot: list = None, kardex_html: str = '') -> str:
    ...
    # Separar productos con stock de los agotados
...
def fetch_kardex_today_supabase(supabase_url: str, supabase_key: str) -> list:
    """Lee los movimientos de HOY desde Supabase (nuevo sistema)."""
    from datetime import timezone, timedelta
    today = datetime.now(timezone.utc).astimezone(timezone(timedelta(hours=-5))).date().isoformat()
    url = (supabase_url + '/rest/v1/kardex'
           '?fecha=eq.' + today +
           '&order=hora.asc'
           '&select=hora,codigo_producto,descripcion_producto,tipo,cantidad,responsable,remision,lote,colada')
    ...
    try:
        with urllib.request.urlopen(req) as resp:
            ...
            print('Supabase kardex hoy: ' + str(len(rows)) + ' movimientos')
            # Convertir a formato compatible con build_kardex_html
            result = []
            for k in rows:
                result.append({
                    'fecha':    today,
                    'hora':     str(k.get('hora',''))[:5],
                    'producto': str(k.get('codigo_producto','')),
                    'desc':     str(k.get('descripcion_producto','')),
                    'tipo':     str(k.get('tipo','')).upper(),
                    'cant':     k.get('cantidad',''),
                    'stock':    '',
                    'costo':    '',
                    'resp':     str(k.get('responsable','')),
                    'remision': str(k.get('remision','')),
                    'lote':     str(k.get('lote','')),
                    'colada':   str(k.get('colada','')),
                })
            return result
    except Exception as e:
        print('Supabase kardex error: ' + str(e))
        return []
...
if __name__ == '__main__':
    ...
    try:
        if supabase_url and supabase_key:
            movs = fetch_kardex_today_supabase(supabase_url, supabase_key)
        else:
            print("SUPABASE_URL/KEY no configurados, intentando OneDrive...")
